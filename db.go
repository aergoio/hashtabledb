...
type DB struct {
	...
	// (see waitForCompletion). pending is cleared when a command STARTS so a
	// later request can enqueue a follow-up full run while one is in progress.
	workerCmds map[string]*cmdSlot
	// cmdMutex / cmdCond serialize waiters with finishCommand Broadcast.
	// Dedicated (not seqMutex) so command-completion signaling does not ride the
...
type cmdSlot struct {
	pending   bool   // true if a command is sitting in the channel (not yet started)
...
func Open(path string, options ...Options) (*DB, error) {
	...
	db.cmdCond = sync.NewCond(&db.cmdMutex)
...
func (db *DB) setKvOnIndex(key, value []byte, dataOffset uint64, dataSize uint32) error {
	// Hash the key with initial salt
	hash := hashKey(key, InitialSalt)
...
func (db *DB) resolveExistingData(existingDataOffset uint64, existingDataSize uint16, key, value []byte, dataOffset uint64, dataSize uint32) (
	action dataOffsetAction, newDataOffset uint64, newDataSize uint32, existingKey []byte, err error,
) {
	// Read the content at the offset into a stack-allocated record: the
	// struct does not escape, so this stays off the heap
	var content Content
	if err := db.readContent(int64(existingDataOffset), existingDataSize, &content); err != nil {
...
func (db *DB) setOnTablePage(tablePage *TablePage, key, value []byte, dataOffset uint64, dataSize uint32, forcedSlot ...int) error {
...
func hybridSubPageLive(info HybridSubPageInfo) bool {
	return int(info.Offset) >= HybridHeaderSize
}

func liveHybridSubPageIDs(hybridPage *HybridPage) []int {
	var ids []int
	for i := 0; i < len(hybridPage.SubPages); i++ {
		if hybridSubPageLive(hybridPage.SubPages[i]) {
			ids = append(ids, i)
		}
	}
	return ids
...
func (db *DB) setOnHybridSubPage(subPage *HybridSubPage, key, value []byte, dataOffset uint64, dataSize uint32) error {
	...
	if int(subPageId) >= len(hybridPage.SubPages) || !hybridSubPageLive(hybridPage.SubPages[subPageId]) {
		return fmt.Errorf("sub-page with index %d not found on page %d (numSub=%d contentSize=%d live=%v)",
	...
	entryIndex, nextPageNumber, nextSubPageId, existingDataOffset, existingDataSize, err := db.findEntryInHybridSubPage(hybridPage, subPageInfo, slot)
	if err != nil {
		return fmt.Errorf("failed to search in hybrid sub-page: %w", err)
	}

	// Slot empty: no sub-page pointer and no data pointer
	if nextPageNumber == 0 && existingDataOffset == 0 {
	...
	if nextPageNumber > 0 {
		...
		if page.pageType == ContentTypeTable {
			...
			entryIndex, reFoundPageNumber, _, _, _, ferr := db.findEntryInHybridSubPage(subPage.Page, &subPage.Page.SubPages[parentSubPageId], slot)
			if ferr != nil {
				return fmt.Errorf("failed to relocate parent entry after child set: %w", ferr)
			}
			if reFoundPageNumber == 0 {
			...
			return db.updateSubPagePointerInHybridSubPage(subPage, entryIndex, nextSubPage.Page.pageNumber, nextSubPage.SubPageId)
		} else {
			return fmt.Errorf("invalid page type: %c", page.pageType)
		}
		...
		case dataOffsetClear:
			...
			return db.removeEntryFromHybridSubPage(subPage, entryIndex)
		case dataOffsetUpdate:
			...
			return db.updateDataOffsetInHybridSubPage(subPage, entryIndex, newDataOffset, newDataSize)
		case dataOffsetCollide:
			...
			*/
			...
			err = db.convertEntryInHybridSubPage(subPage, entryIndex, newSubPage.Page.pageNumber, newSubPage.SubPageId)
			if err != nil {
...
// record offset
func (db *DB) lookupRecordOffset(key []byte, maxReadSequence int64) (uint64, uint16, error) {
	// Hash the key with initial salt
	hash := hashKey(key, InitialSalt)
...
func (db *DB) lookupOffsetInPage(key []byte, pageNumber uint32, subPageId uint8, maxReadSequence int64) (uint64, uint16, error) {
...
// a page pointer
func (db *DB) lookupOffsetInTablePage(key []byte, tablePage *TablePage, maxReadSequence int64, forcedSlot ...int) (uint64, uint16, error) {
...
func (db *DB) lookupOffsetInHybridSubPage(key []byte, hybridPage *HybridPage, subPageId uint8, maxReadSequence int64) (uint64, uint16, error) {
	// Get the sub-page info to get the salt
	if int(subPageId) >= len(hybridPage.SubPages) || !hybridSubPageLive(hybridPage.SubPages[subPageId]) {
	...
	// Search in the specific sub-page
	_, nextPageNumber, nextSubPageId, dataOffset, dataSize, err := db.findEntryInHybridSubPage(hybridPage, subPageInfo, slot)
	if err != nil {
		return 0, 0, fmt.Errorf("failed to search in hybrid sub-page: %w", err)
	}

	if nextPageNumber > 0 {
		// It's a sub-page pointer: look up the entry on the child page this
		// slot points to
...
func (db *DB) appendData(key, value []byte) (uint64, uint32, error) {
	...
	return uint64(fileSize), uint32(totalSize), nil
}
...
func (db *DB) readContentValue(offset uint64, key []byte, dataSize uint16) ([]byte, error) {
	// Check if offset is valid
	if offset >= uint64(db.mainFileSize.Load()) {
	...
	if dataSize != 0 && dataSize != 0xffff {
		var data []byte
		if db.mainMmapEnabled {
			data, _ = db.mainMmapSlice(int64(offset), int(dataSize))
		}
		if data == nil {
			data = make([]byte, int(dataSize))
			n, err := db.mainFile.ReadAt(data, int64(offset))
			if err != nil && err != io.EOF {
	...
	var content Content
	if err := db.readContent(int64(offset), dataSize, &content); err != nil {
...
// The callback receives entryIndex, slot, pageNumber, subPageId, dataOffset, and dataSize
// Returns true to continue or false to stop
func (db *DB) iterateHybridSubPageEntries(hybridPage *HybridPage, subPageInfo *HybridSubPageInfo, callback func(entryIndex int, slot int, pageNumber uint32, subPageId uint8, dataOffset uint64, dataSize uint16) bool) error {
	...
	// cursors so each step just reads and advances
	slotPos := int(subPageInfo.Offset) + HybridSubPageHeaderSize
	ptrPos := slotPos + 2*count

	// Read entries until we reach the end of the sub-page
	for i := 0; i < count; i++ {
		slot := int(binary.LittleEndian.Uint16(hybridPage.data[slotPos:]))
		slotPos += 2

		value := binary.LittleEndian.Uint64(hybridPage.data[ptrPos:])
		ptrPos += 8
		...
		var dataSize uint16
		if value>>63 == 1 {
			// Sub-page pointer: pageNumber (flag in high bits) + subPageId
			pageNumber = uint32((value >> 8) & 0x7FFFFFFF)
			subPageId = uint8(value)
		} else {
			// Data pointer: 47-bit offset + 16-bit size
			dataOffset = value >> 16
			dataSize = uint16(value)
		}

		// Call the callback with the entry information (matching original pattern)
		if !callback(i, slot, pageNumber, subPageId, dataOffset, dataSize) {
			break
		}
	}
...
// hybridDataPtrWord packs a data pointer for the sub-page pointer array:
// 47-bit offset in the high bits (bit 63 clear = data pointer), 16-bit size
// in the low bits
func hybridDataPtrWord(dataOffset uint64, dataSize uint32) uint64 {
	if dataSize >= 0xffff {
		dataSize = 0xffff
	}
	return dataOffset<<16 | uint64(dataSize)
}

// hybridSubPtrWord packs a sub-page pointer for the sub-page pointer array:
// bit 63 set, then the 31-bit page number and the sub-page id
func hybridSubPtrWord(pageNumber uint32, subPageId uint8) uint64 {
	return uint64(1)<<63 | uint64(pageNumber)<<8 | uint64(subPageId)
}

func putHybridDataOffset(dst []byte, dataOffset uint64, dataSize uint32) error {
	if len(dst) < 8 {
		return fmt.Errorf("destination is too small for data pointer")
	}
	if dataOffset > 0x7FFFFFFFFFFF {
	...
	binary.BigEndian.PutUint64(dst, dataOffset<<16|uint64(dataSize))
	return nil
}

// getHybridDataOffset reads a hybrid data-record offset and data size
func getHybridDataOffset(src []byte) (uint64, uint16) {
	v := binary.BigEndian.Uint64(src)
	return v >> 16, uint16(v)
}
...
func (db *DB) addEntryToNewHybridSubPage(parentSalt uint8, key []byte, dataOffset uint64, dataSize uint32) (*HybridSubPage, error) {
...
func (db *DB) addEntriesToNewHybridSubPage(parentSalt uint8, entries []HybridEntry) (*HybridSubPage, error) {
	...
	// Entry format: slot(u16 LE) + data_pointer(8) = 10 bytes per entry
	subPageSize := len(entries) * 10 // Size of the data (excluding the header)
	totalSubPageSize := HybridSubPageHeaderSize + subPageSize // 4 bytes header + data
	...
	releaseReserve := func() {
		if hybridPage != nil && int(subPageID) < len(hybridPage.SubPages) &&
			hybridPage.SubPages[subPageID].Offset == hybridSubPageIDReserved {
			hybridPage.SubPages[subPageID] = HybridSubPageInfo{}
		}
	}
	...
	// Write all entries data directly after the header: u16 slots first,
	// then u64 pointer words
	slotsPos := offset + HybridSubPageHeaderSize
	ptrsPos := slotsPos + 2*len(entries)
	...
	for _, entry := range entries {
		...
		// Write slot (fixed little-endian u16)
		binary.LittleEndian.PutUint16(hybridPage.data[slotsPos:], uint16(slot))
		slotsPos += 2

		// Write the data pointer word (bit 63 clear = data pointer)
		binary.LittleEndian.PutUint64(hybridPage.data[ptrsPos:], hybridDataPtrWord(entry.DataOffset, entry.DataSize))
		ptrsPos += 8
	}
...
func (db *DB) addEntryToHybridSubPage(subPage *HybridSubPage, slot int, key []byte, dataOffset uint64, dataSize uint32) error {
	var err error
	...
	count := int(subPageInfo.Size) / 10
	slotsEnd := currentSubPageStart + 2*count

	// Step 2: Open room for the new slot in the slot array and the new pointer
	// word at the end of the pointer array. Everything after the slot array
	// (pointer words and subsequent sub-pages) moves right by 10 bytes, then
	// the pointer words settle 2 bytes right of their old position
	if slotsEnd < hybridPage.ContentSize {
		copy(hybridPage.data[slotsEnd+10:], hybridPage.data[slotsEnd:hybridPage.ContentSize])
	}
	if count > 0 {
		copy(hybridPage.data[slotsEnd+2:], hybridPage.data[slotsEnd+10:slotsEnd+10+8*count])
	}

	// Step 3: Serialize the new entry in the parallel arrays
	binary.LittleEndian.PutUint16(hybridPage.data[slotsEnd:], uint16(slot))
	binary.LittleEndian.PutUint64(hybridPage.data[slotsEnd+2+8*count:], hybridDataPtrWord(dataOffset, dataSize))
	...
	for i := 0; i < len(hybridPage.SubPages); i++ {
		if hybridSubPageLive(hybridPage.SubPages[i]) && hybridPage.SubPages[i].Offset > subPageInfo.Offset {
...
func (db *DB) removeEntryFromHybridSubPage(subPage *HybridSubPage, entryIndex int) error {
	...
	subPageEnd := slotsStart + int(subPageInfo.Size)

	// Close the slot gap in the slot array
	copy(hybridPage.data[slotsStart+2*entryIndex:], hybridPage.data[slotsStart+2*entryIndex+2:slotsEnd])

	// Close the pointer gap in the pointer array (which shifts left by 2 with the slots)
	ptrsOldStart := slotsEnd
	ptrsNewStart := slotsEnd - 2
	copy(hybridPage.data[ptrsNewStart:], hybridPage.data[ptrsOldStart:ptrsOldStart+8*entryIndex])
	copy(hybridPage.data[ptrsNewStart+8*entryIndex:], hybridPage.data[ptrsOldStart+8*entryIndex+8:ptrsOldStart+8*count])

	// Move all data after the sub-page left by 10 bytes to fill the gap
	if subPageEnd < hybridPage.ContentSize {
		copy(hybridPage.data[subPageEnd-10:], hybridPage.data[subPageEnd:hybridPage.ContentSize])
	}

	// Update the sub-page size
	newSubPageSize := int(subPageInfo.Size) - 10
	...
	for i := 0; i < len(hybridPage.SubPages); i++ {
		if hybridSubPageLive(hybridPage.SubPages[i]) && hybridPage.SubPages[i].Offset > subPageInfo.Offset {
			hybridPage.SubPages[i].Offset -= 10
		}
	}

	// Update the hybrid page content size
	hybridPage.ContentSize -= 10
...
func (db *DB) updateDataOffsetInHybridSubPage(subPage *HybridSubPage, entryIndex int, dataOffset uint64, dataSize uint32) error {
	// Get a writable version of the page
	if err := db.getWritableSubPage(subPage); err != nil {
		return fmt.Errorf("failed to get writable page: %w", err)
	}
	...
	// Rewrite the pointer word in the pointer array (bit 63 clear = data pointer)
	slotsStart := int(hybridPage.SubPages[subPage.SubPageId].Offset) + HybridSubPageHeaderSize
	ptrPos := slotsStart + 2*(int(hybridPage.SubPages[subPage.SubPageId].Size)/10) + 8*entryIndex
	binary.LittleEndian.PutUint64(hybridPage.data[ptrPos:], hybridDataPtrWord(dataOffset, dataSize))
...
func (db *DB) updateSubPagePointerInHybridSubPage(subPage *HybridSubPage, entryIndex int, pageNumber uint32, subPageId uint8) error {
	...
	// Rewrite the pointer word in the pointer array (bit 63 set = sub-page pointer)
	slotsStart := int(hybridPage.SubPages[subPage.SubPageId].Offset) + HybridSubPageHeaderSize
	ptrPos := slotsStart + 2*(int(hybridPage.SubPages[subPage.SubPageId].Size)/10) + 8*entryIndex
	binary.LittleEndian.PutUint64(hybridPage.data[ptrPos:], hybridSubPtrWord(pageNumber, subPageId))
...
// convertEntryInHybridSubPage converts a data pointer entry to a sub-page
// pointer entry. Both live in the same u64 pointer word, so this is an
...
func (db *DB) convertEntryInHybridSubPage(subPage *HybridSubPage, entryIndex int, pageNumber uint32, subPageId uint8) error {
	...
	// Rewrite the pointer word (bit 63 set = sub-page pointer)
	slotsStart := int(subPageInfo.Offset) + HybridSubPageHeaderSize
	ptrPos := slotsStart + 2*(int(subPageInfo.Size)/10) + 8*entryIndex
	binary.LittleEndian.PutUint64(hybridPage.data[ptrPos:], hybridSubPtrWord(pageNumber, subPageId))
...
func (db *DB) setTableEntry(tablePage *TablePage, slot int, pageNumber uint32, subPageId uint8, dataOffset uint64) error {
...
func (db *DB) getTableEntry(tablePage *TablePage, slot int) (uint32, uint8, uint64) {
	if slot < 0 || slot >= TableEntries {
		return 0, 0, 0
	}
	...
	if isDataOffset {
		// Direct data offset: 39-bit address
		dataOffset := (uint64(word) << 8) | uint64(tablePage.data[offset+4])
...
func (db *DB) convertHybridSubPageToTablePage(subPage *HybridSubPage, newSlot int, key []byte, newDataOffset uint64, newDataSize uint32) error {
	var err error
	if err = db.getWritableSubPage(subPage); err != nil {
	...
	var walkErr error
	err = db.iterateHybridSubPageEntries(srcHybridPage, &srcHybridPage.SubPages[SubPageId], func(entryIndex int, slot int, pageNumber uint32, subPageId uint8, dataOffset uint64, dataSize uint16) bool {
		if slot == newSlot {
		...
		if pageNumber > 0 {
			...
			if setErr := db.setTableEntry(tablePage, slot, pageNumber, subPageId, 0); setErr != nil {
				walkErr = setErr
				return false
			}
		} else {
			// Store the data offset directly, or in a child sub-page if it exceeds 39 bits
			if setErr := db.setTableSlotDataOffset(tablePage, slot, nil, dataOffset, uint32(dataSize)); setErr != nil {
				walkErr = setErr
				return false
			}
		}
...
func (db *DB) setTableSlotDataOffset(tablePage *TablePage, slot int, key []byte, dataOffset uint64, dataSize uint32) error {
	...
	if len(key) == 0 {
		var content Content
		if err := db.readContent(int64(dataOffset), 0xffff, &content); err != nil {
...
func (db *DB) moveSubPageToNewHybridPage(subPage *HybridSubPage, slot int, dataOffset uint64, dataSize uint32) error {
	...
	releaseReserve := func() {
		if newHybridPage != nil && int(newSubPageID) < len(newHybridPage.SubPages) &&
			newHybridPage.SubPages[newSubPageID].Offset == hybridSubPageIDReserved {
	...
	// (the parallel slot/pointer arrays move over unchanged)
	...
	// Append the new entry: its slot joins the slot array and its pointer
	// word joins the pointer array (both inside the copied block)
	count := int(subPageInfo.Size) / 10
	slotsEnd := dataPos + 2*count
	copy(newHybridPage.data[slotsEnd+2:], newHybridPage.data[slotsEnd:slotsEnd+8*count])
	binary.LittleEndian.PutUint16(newHybridPage.data[slotsEnd:], uint16(slot))
	binary.LittleEndian.PutUint64(newHybridPage.data[slotsEnd+2+8*count:], hybridDataPtrWord(dataOffset, dataSize))
	dataPos += 2*count + 2 + 8*count + 8
...
func (db *DB) removeSubPageFromHybridPage(hybridPage *HybridPage, SubPageId uint8) {
	// Get the sub-page info
	if int(SubPageId) >= len(hybridPage.SubPages) {
		return
	}
	subPageInfo := hybridPage.SubPages[SubPageId]
	...
	// Reserved-but-unwritten slot: just clear the reservation
	if !hybridSubPageLive(subPageInfo) {
		hybridPage.SubPages[SubPageId] = HybridSubPageInfo{}
		db.markPageDirty(hybridPage)
		return
	}
	...
	for currentOffset < fileSize {
		...
		for currentOffset < fileSize && recordsInBatch < batchSize {
			...
			if content.data[0] == ContentTypeData {
				if err := db.setKvOnIndex(content.key, content.value, uint64(currentOffset), uint32(len(content.data))); err != nil {
					db.rollbackTransaction()
...
func (db *DB) reindexContent(lastIndexedOffset int64) error {
	...
	for currentOffset < db.mainFileSize.Load() {
		...
		if content.data[0] == ContentTypeData {
			...
			err := db.setKvOnIndex(content.key, content.value, uint64(currentOffset), uint32(len(content.data)))
			if err != nil {
				return fmt.Errorf("failed to set kv on index: %w", err)
			}
...
// cmdSlot helpers. pending is cleared when a command STARTS (beginCommand) so a
// later fresh request can enqueue a follow-up full run while one is in progress.
// completed is the last finished request id: a run that began with requestId
// covers all requests up to requestId when it finishes.

func (db *DB) cmdSlot(cmd string) *cmdSlot {
	s := db.workerCmds[cmd]
	if s == nil {
		s = &cmdSlot{}
		db.workerCmds[cmd] = s
...
// getCurrentRequestId returns the current request id for a command without
// enqueueing. Used when work is requested indirectly (e.g. checkpointWAL
// requests checkpoint_clean) and the caller needs to see whether the id advanced.
func (db *DB) getCurrentRequestId(cmd string) uint64 {
	db.seqMutex.Lock()
	id := db.cmdSlot(cmd).requested
	db.seqMutex.Unlock()
	return id
}
...
// finishCommand records that a run covering requestId has finished and wakes
// waiters. pending was already cleared in beginCommand.
//
...
func (db *DB) finishCommand(cmd string, requestId uint64) {
	db.cmdMutex.Lock()
	db.seqMutex.Lock()
	s := db.cmdSlot(cmd)
	s.running = false
	if requestId > s.completed {
		s.completed = requestId
	}
	db.seqMutex.Unlock()
	db.cmdCond.Broadcast()
	db.cmdMutex.Unlock()
}
...
func (db *DB) requestClean(fresh bool) uint64 {
	return db.requestCleaner("clean", fresh)
}

func (db *DB) requestFlush(fresh bool) uint64 {
	return db.requestFlusher("flush", fresh)
}

func (db *DB) requestCheckpoint(fresh bool) uint64 {
	return db.requestFlusher("checkpoint", fresh)
}
...
//
// Holds cmdMutex across the condition check and Wait so finishCommand's
// Broadcast cannot be lost in a gap. writeMutex stays held by set().
func (db *DB) waitForCompletion(cmd string, requestId uint64) {
	if requestId == 0 {
		return
	}
	db.cmdMutex.Lock()
	for {
		db.seqMutex.Lock()
		done := db.cmdSlot(cmd).completed >= requestId
		db.seqMutex.Unlock()
		if done {
			break
		}
...
func (db *DB) startFlusherThread() {
	...
	db.flusherThreadWaitGroup.Add(1)

	go func() {
		// Ensure the wait group is decremented when the goroutine exits
		defer db.flusherThreadWaitGroup.Done()
		...
		for {
			select {
			case cmd, ok := <-db.flusherThreadChannel:
				if !ok {
					// Channel closed, exit
					return
				}
				...
				switch cmd {
				case "flush":
					requestId := db.beginCommand("flush")
				flush_again:
				...
				case "checkpoint":
					...
					db.checkpointWAL()
					db.readMutex.RUnlock()
					db.finishCommand("checkpoint", requestId)

				case "exit":
					return

				default:
					debugPrint("Unknown flusher thread command: %s\n", cmd)
				}
			...
			case <-timer.C:
				...
				// is woken via finishCommand when the flush completes).
				// requestFlush(false) is a no-op if a flush is already pending.
...
// startCleanerThread starts the cleaner thread for cache cleanup operations
func (db *DB) startCleanerThread() {
	// Add 1 to the wait group before starting the goroutine
	db.cleanerThreadWaitGroup.Add(1)
		...
		for {
			...
			case cmd, ok := <-db.cleanerThreadChannel:
				...
				case "clean":
					...
					requestId := db.beginCommand("clean")
					// Coordinate with Close() using readMutex
					db.readMutex.RLock()
				...
				case "checkpoint_clean":
					...
					db.readMutex.RUnlock()
					db.finishCommand("checkpoint_clean", requestId)

				case "adaptive_cache":
					requestId := db.beginCommand("adaptive_cache")
					// Adaptive cache management triggered by cache growth request
					db.adaptiveCacheManager()
					db.finishCommand("adaptive_cache", requestId)
